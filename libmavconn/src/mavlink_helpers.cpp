...
{
	printf("mavconn: Initialize message_entries map");
	...
	auto load = [&](const char *dialect, const mavlink::mavlink_msg_entry_t & e) {
		...
		if (it != message_entries.end()) {
			if (memcmp(&e, it->second, sizeof(e)) != 0) {
				printf("mavconn: init: message from %s, MSG-ID %d ignored! Table has different entry.", dialect, e.msgid);
			}
			else {
				printf("mavconn: init: message from %s, MSG-ID %d in table.", dialect, e.msgid);
			}
		}
		else {
			printf("mavconn: init: add message entry for %s, MSG-ID %d", dialect, e.msgid);
